...
# Validate partner and client exist
$partnerDir = Join-Path $toolkitDir "partners\$($Partner.ToLower())"
$clientDir = Join-Path $toolkitDir "clients\$($Client.ToLower().Replace(' ', '-').Replace('_', '-'))"

if (!(Test-Path $partnerDir)) {
    Write-Error "Partner '$Partner' not found. Available partners:"
    Get-ChildItem (Join-Path $toolkitDir "partners") -Directory | ForEach-Object { Write-Host "  - $($_.Name)" -ForegroundColor Yellow }
    exit 1
}

if (!(Test-Path $clientDir)) {
    Write-Error "Client '$Client' not found. Available clients:"
    Get-ChildItem (Join-Path $toolkitDir "clients") -Directory | ForEach-Object { Write-Host "  - $($_.Name)" -ForegroundColor Yellow }
    exit 1
}
